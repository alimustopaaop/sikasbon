...
class Pimpinan extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        // Validasi jika role bukan Pimpinan atau super admin
        if ($role != 'Pimpinan' && $role != 'Super Admin' && $role != 'Admin') {
    ...
    public function pengajuan()
    {
        ...
        // var_dump($id);die;
        error_reporting(0);
        $list = $this->db->query("SELECT * FROM v_ajuan_status where status <> '1' and (id_user = '$id' OR pihak_2 = '$id' OR mengetahui_1 = '$id' OR mengetahui_2 = '$id') ORDER BY id_ajuan DESC")->result();
        $jabatan = $this->db->query("Select * from jabatan")->result();
        $pihak = $this->db->query("SELECT * FROM users WHERE id <> ?", [$id])->result();
    ...
    public function ubah_status()
    {
        ...
        // Memastikan hanya akses POST yang diperbolehkan
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $cekttd = $this->db->query("SELECT COUNT(*) AS jumlah
            FROM step1_ajuan
            WHERE ttd_1 IS NOT NULL
              AND ttd_2 IS NOT NULL
              AND ttd_3 IS NOT NULL
              AND ttd_4 IS NOT NULL
              AND  id_ajuan = '$id_ajuan'")->row();

            if ($cekttd->jumlah == 0) {
                # code...
                $this->session->set_flashdata('sweet_error', 'Mohon Maaf masih ada yang belum ditanda tangan. Silahkan lengkapi terlebih dahulu ');
                redirect('pimpinan/detail_pimpinan?d=' . $id_ajuan . '&s=' . $hash);
            }

            // Mengupdate data di tabel "ajuan" berdasarkan "id_ajuan"
            $data = array(
                'id_ajuan' => $id_ajuan,
                'status' => $status,
                'catatan' => $catatan,
            ...
            $this->session->set_flashdata('sweet_success', 'Data status berhasil diubah...');
            // Redirect ke halaman atau melakukan operasi lain setelah berhasil mengupdate data
            redirect('pimpinan/pengajuan');
        } else {
            // Jika bukan metode POST, bisa diarahkan ke halaman lain atau menampilkan pesan error
    ...
    public function simpan_step1()
    {
        ...
        if ($result) {
            $this->session->set_flashdata('sweet_success', 'Data berhasil disimpan...');
            redirect('pimpinan/detail_ajuan?d=' . $id_ajuan . '&s=' . $hash);
        } else {
            $this->session->set_flashdata('sweet_error', 'Data Gagal diubah...');
            redirect('pimpinan/detail_ajuan?d=' . $id_ajuan . '&s=' . $hash);
        }
    ...
    public function simpan_step2()
    {
        ...
        if ($result) {
            $this->session->set_flashdata('sweet_success', 'Data berhasil disimpan...');
            redirect('pimpinan/detail_belanja?d=' . $id_ajuan . '&s=' . $hash);
        } else {
            $this->session->set_flashdata('sweet_error', 'Data Gagal diubah...');
            redirect('pimpinan/detail_belanja?d=' . $id_ajuan . '&s=' . $hash);
        }
    ...
    public function detail_pengeluaran()
    {
        ...
        if ($step2->num_rows() < 1) {
            ...
            redirect('pimpinan/detail_belanja?d=' . $d . '&s=' . $s);
        }
        $nilai = $step2->row();
...
// Menjumlahkan nilai-nilai tersebut
    ...
    public function simpan_step3()
    {
        ...
        if ($result) {
            $this->session->set_flashdata('sweet_success', 'Data berhasil disimpan...');
            redirect('pimpinan/detail_pengeluaran?d=' . $id_ajuan . '&s=' . $hash);
        } else {
            $this->session->set_flashdata('sweet_error', 'Data Gagal diubah...');
            redirect('pimpinan/detail_pengeluaran?d=' . $id_ajuan . '&s=' . $hash);
        }
    ...
    public function simpan_step4()
    {
        ...
        if ($result) {
            $this->session->set_flashdata('sweet_success', 'Data berhasil disimpan...');
            redirect('pimpinan/detail_bendahara?d=' . $id_ajuan . '&s=' . $hash);
        } else {
            $this->session->set_flashdata('sweet_error', 'Data Gagal diubah...');
            redirect('pimpinan/detail_bendahara?d=' . $id_ajuan . '&s=' . $hash);
        }
    ...
    public function detail_pimpinan()
    {
        ...
        if ($step1 < 1) {
            ...
            redirect('pimpinan/detail_ajuan?d=' . $d . '&s=' . $s);
        } elseif ($step2 < 1) {
            # code...
            $this->session->set_flashdata('sweet_error', 'Mohon maaf silahkan lengkapi menu Belanja (PEMENDAGRI64)...');
            redirect('pimpinan/detail_belanja?d=' . $d . '&s=' . $s);
